...
class LearnObject extends StatefulWidget {
  const LearnObject({Key? key}) : super(key: key);

  @override
  _LearnObjectState createState() => _LearnObjectState();
}

class _LearnObjectState extends State<LearnObject> {
  int isPage = 0;
  final int _pageLenght = imagesObj.length + trueObj.length;
          ...
          children: [
            ...
            SizedBox(
              ...
              child: PageView.builder(
                physics: isPage < imagesObj.length
            ...
            isPage < imagesObj.length
                ? Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: buildIndicators(
                      height: _height,
                      isPage: isPage,
                      isI: imagesObj.length + 1,
                      horiHei: 150,
                    ),
                  )
                : Container(),
    ...
    for (var i = 0; i < imagesObj.length; i++) {
      _object.add(
        buildObject(
          ...
          head: "image",
          image: imagesObj[i],
          nom: tranlateObj[i]['name'],
          traduction: tranlateObj[i]['translate'],
        ),
      );
    }
    List<Widget> _exercice = [];
    for (var i = 0; i < trueObj.length; i++) {
      _exercice.add(
        _buildExercice(
          width: width,
          height: height,
